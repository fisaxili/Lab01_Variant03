        ...
        static double ReadTemperature()
        {
            while (true)
            {
                try
                {
                    ...
                    // Проверка нижнего физического ограничения
                    if (celsius < -273.15)
                    {
                    ...
                    //Проверка верхнего физического ограничения
                    if (celsius > 15000000)
                    {
                        Console.WriteLine("Ошибка.Температруа не должна превышать 15 млн °C (слишком высокая температура).");
                        continue;
                    }
                    return celsius;
                }
